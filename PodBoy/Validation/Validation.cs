...
namespace PodBoy.Validation
{
    ...
    public class ReactiveValidatedObject : ReactiveObject, IDataErrorInfo
    {
        ...
        public string this[string columnName]
        {
            get
            {
                ...
                _ValidationObservable.OnNext(new ObservedChange<object, bool>(this, expression.Body, ret != null));
        ...
        private static readonly MemoizingMRUCache<Type, Dictionary<string, PropertyExtraInfo>> allValidatedProperties =
            new MemoizingMRUCache<Type, Dictionary<string, PropertyExtraInfo>>(
                (x, _) => PropertyExtraInfo.CreateFromType(x).ToDictionary(k => k.PropertyName, v => v), 5);
        ...
        private string getPropertyValidationError(string propName)
        {
            ...
            foreach (var v in pei.ValidationAttributes)
            {
                try
                {
                    ...
                    v.Validate(getter(this, new object[]
                    {}), ctx);
                }
                catch (Exception ex)
                {
                    this.Log().Info("{0:X}.{1} failed validation: {2}", this.GetHashCode(), propName, ex.Message);
                    return ex.Message;
                }
            }
    ...
    internal class PropertyExtraInfo : IComparable
    {
        ...
        public static PropertyExtraInfo CreateFromTypeAndName(Type type,
            string propertyName,
        ...
        public static PropertyExtraInfo[] CreateFromType(Type type)
        {
            return
                type.GetProperties(BindingFlags.Instance | BindingFlags.Public).Select(
                    x => CreateFromTypeAndName(type, x.Name, true)).Where(x => x != null).ToArray();
        }
    ...
    public abstract class ValidationBase : ValidationAttribute
    {
        ...
        protected ValidationResult isValidViaNullOrBlank(object value, ValidationContext ctx)
        {
            ...
            return new ValidationResult(String.Format("{0} is blank", ctx.DisplayName ?? "The value"));
        }
    ...
    public class ValidatesViaMethodAttribute : ValidationBase
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            bool result = (bool) mi.Invoke(validationContext.ObjectInstance, new[]
            {
                value
            });
